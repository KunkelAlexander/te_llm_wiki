---
title: "E-fuels financing and infrastructure"
type: topic
section: "Energy & Fuels"
summary: >
  T&E's case that e-fuels (green hydrogen derivatives) are the only aviation/shipping fuel
  meeting sustainability, scalability and sovereignty criteria, but that a financing gap —
  not technology — is what's stalling projects, and its push for a Revenue Certainty Mechanism
  funded from ETS revenue to close it.
first_seen: 2025-02-13
last_updated: 2025-09-09
---

# E-fuels financing and infrastructure

*Updated: 2025-09-09*

Across mid-to-late 2025, T&E published a cluster of reports making the same underlying argument
about e-fuels (hydrogen-derived e-kerosene, e-ammonia, e-methanol, etc. for aviation and
shipping): the technology and project pipeline exist, but financing — specifically, the lack of a
mechanism to guarantee revenue for first-of-a-kind plants — is the binding constraint, and closing
that gap should be a priority use of EU carbon-pricing revenue.

**Contents:** [An early financing mechanism: the Hydrogen Clearing House](#an-early-financing-mechanism-the-hydrogen-clearing-house) · [The case for e-fuels over the alternatives](#the-case-for-e-fuels-over-the-alternatives) · [E-kerosene specifically: Europe's lead is not secure](#e-kerosene-specifically-europes-lead-is-not-secure) · [Related dossiers](#related-dossiers)

## An early financing mechanism: the Hydrogen Clearing House

[A European Hydrogen Clearing House for green maritime and aviation e-fuels](../sources/a-european-hydrogen-clearing-house-for-green-maritime-and-aviation-e-fuels.md)
(February 2025) is T&E's earliest concrete financing proposal in this cluster: even with EU and
national subsidies (the Innovation Fund, the Hydrogen Bank), e-fuels can't yet compete with fossil
LNG or biofuels on cost — the H2 Bank's first pilot auction awarded only €0.37-0.48/GJ, not enough
to close the price gap for shipping, risking projects being delayed, cancelled, or diverted to
other hydrogen end-uses. A clearing house would match green hydrogen supply and demand across
sectors to reduce that risk. This proposal predates, and is narrower than, the Revenue Certainty
Mechanism T&E proposed later in the year (see below).

## The case for e-fuels over the alternatives

[Unlocking e-fuels for aviation and shipping and clean transport infrastructure](../sources/unlocking-e-fuels-for-aviation-and-shipping-and-clean-transport-infrastructure.md)
(September 2025, a response to the EU's Sustainable Transport Investment Plan (STIP) consultation)
argues e-fuels (RFNBOs) are the only aviation/shipping fuel option that is simultaneously
sustainable, scalable and sovereign — fossil LNG and crop-based biofuels fail on sustainability
(see [Biofuels](biofuels.md)), and low-carbon "blue" hydrogen falls short on genuine climate
benefit (see [Hydrogen policy](hydrogen-policy.md)). The report proposes a Revenue Certainty
Mechanism — double-sided auctions or contracts-for-difference — funded by earmarking 25% of
shipping and aviation ETS revenue, which it estimates could mobilise €24-30bn for shipping and
€13-38bn for aviation e-fuels over 2030-2039. It also flags an immediate funding gap: the
Alternative Fuels Infrastructure Facility (AFIF) needs €1.25-1.5bn in bridge funding to avoid a
2026-27 shortfall, and existing mechanisms — the EU Hydrogen Bank, ReFuelEU SAF allowances, EIB
financing — are judged inadequate on their own (Hydrogen Bank subsidies awarded so far, €0.45-0.63
per kg H2, are far below the real cost gap of €5-11/kg; four hydrogen projects totalling 1.3 GW
have already withdrawn from the scheme).

## E-kerosene specifically: Europe's lead is not secure

[Unlocking e-SAF's potential for the EU competitiveness and energy independence](../sources/unlocking-e-saf-s-potential-for-the-eu-competitiveness-and-energy-independence.md)
(June 2025) makes the funding-priority case directly to the EU's Sustainable Transport Investment
Plan (STIP): of the SAF options, e-SAF is the most scalable and sustainable long-term bet, whereas
biofuel-derived SAF is either not scalable (limited domestic waste-oil supply) or, where imported,
increasingly linked to fraud (see [Biofuels](biofuels.md)) — so STIP funding should prioritise
e-SAF specifically, which T&E also frames as a competitiveness and energy-independence play, not
just a climate one.

[Spotlight on e-SAF](../sources/spotlight-on-e-saf.md) (June 2025) narrows the same argument to
aviation e-kerosene, which it credits with over 90% lifecycle emission-reduction potential and
frames as industrially strategic for Europe's hydrogen economy — but notes no large-scale plant
is yet operational anywhere in the world, and none in Europe have reached a Final Investment
Decision (FID). Europe leads on project announcements (41 large-scale projects as of May 2025, up
from 27 in January 2024, with combined capacity of 2.8 Mt/year — roughly 3x the 2030 ReFuelEU
sub-mandate of 1.2% e-kerosene in jet fuel), but risks losing that head start: China has 11
large-scale plants in development (mostly state-owned), and the US secured the first global
large-scale e-SAF FID (Infinium's "Project Roadrunner" in Texas, targeting 2027 operation, with an
offtake deal for 7,500 tonnes/year over 10 years from IAG). T&E puts the investment need at
€10-20bn by 2030 (at €1-2bn per plant) and notes oil majors — who dominate jet fuel supply and
have a combined €130-150bn of 2025-2027 capital budgets across Shell, bp and TotalEnergies — have
so far committed minimal capital to e-SAF specifically.

Industry itself is pushing the same funding mechanism T&E proposes:
[Open letter: E-SAF tender funded by national governments to unlock the first projects in Europe](../sources/open-letter-e-saf-tender-funded-by-national-governments-to-unlock-the-first-proj.md)
(June 2025) has 40+ companies across the SAF value chain, coordinated via Project SkyPower, call on
EU national governments to jointly fund a pilot double-sided-auction e-SAF tender — corroborating
from the supply side that a Revenue Certainty Mechanism-style auction, not just subsidy, is what
first-of-a-kind e-SAF plants need to reach FID.

[E-fuels as the key to Germany's competitiveness and industrial leadership](../sources/e-fuels-als-schl-ssel-f-r-deutschlands-wettbewerbsf-higkeit-und-industrielle-f-h.md)
(open letter, Germany, September 2025) makes the same case from a single-country angle: Germany
already has a head start on e-fuels — research funding, first plants, and the world's first
national e-kerosene blending target — but its industrial ramp-up is stalling on high electricity
prices, the scrapping of national power-to-liquid quotas, and a €2bn funding cut, echoing this
page's broader financing-gap argument at national scale.

## Related dossiers

This financing argument underpins T&E's positions in
[Aviation](aviation-decarbonisation.md) (why SAF/e-kerosene mandates
need real supply behind them) and
[IMO shipping fuel standard](imo-shipping-fuel-standard.md) (why ETS revenue earmarking matters
for shipping e-fuels specifically) — see those pages for the traffic-growth and shipping-politics
angles this page doesn't cover.
